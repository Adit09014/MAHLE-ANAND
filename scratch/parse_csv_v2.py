...
csv_data = """Emp No,Display Name,Work Email,Department,Location,Designation
001332,Shekhar Kanifnath Khedekar,shekhar.khedekar@mahle.com,Production,PUNE,Asst. Manager
...
records = list(reader)
valid_records = [r for r in records if r.get('Emp No') and r['Emp No'].strip()]

print(f"Total valid CSV records: {len(valid_records)}")

# Write complete SQL MERGE script
output_sql = """-- ====================================================================
-- FULL BULK EMPLOYEE SEED SCRIPT FOR MAHLE ANAND FILTER SYSTEM
-- Generated directly from User CSV (Total Records: {})
-- ====================================================================
USE [Rewards];
GO
...
""".format(len(valid_records))

# Chunk size 50
chunk_size = 50
for i in range(0, len(valid_records), chunk_size):
    chunk = valid_records[i:i+chunk_size]
    output_sql += "MERGE dbo.Employees AS target\nUSING (VALUES\n"
    value_lines = []
    for r in chunk:
        emp_no = r['Emp No'].strip()
        name = r['Display Name'].strip().replace("'", "''")
        email = r['Work Email'].strip().replace("'", "''")
        dept = r['Department'].strip().replace("'", "''")
        loc = r['Location'].strip().replace("'", "''")
        desig = r['Designation'].strip().replace("'", "''")
        value_lines.append(f"  (N'{emp_no}', N'{name}', N'{email}', N'{dept}', N'{loc}', N'{desig}')")
    output_sql += ",\n".join(value_lines)
    output_sql += """
...
output_sql += """-- 2. Seed Default Passwords for new employees
...
GO
"""
output_sql += "PRINT 'Bulk employee seed completed successfully! Total records: " + str(len(valid_records)) + "';\nGO\n"

with open(r"d:\ADIT\PROJECT AWARD\my-app\scripts\seed_employees.sql", "w", encoding="utf-8") as f:
    f.write(output_sql)

print(f"Successfully generated seed_employees.sql with {len(valid_records)} employee records.")
